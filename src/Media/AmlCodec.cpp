...
void AmlCodec::InternalOpen(VideoFormatEnum format, int width, int height, double frameRate, bool hasAudioSync)
{
	...
	{
		case VideoFormatEnum::AV1:
			printf("AmlVideoSink - VIDEO/AV1\n");

			amlFormat = VFORMAT_AV1;
			am_sysinfo.format = VIDEO_DEC_FORMAT_AV1;
			break;
			
		case VideoFormatEnum::VP9:
			printf("AmlVideoSink - VIDEO/VP9\n");
	...
	*/
	...
	{
		r = ioctl(cntl_handle, AMSTREAM_IOC_SYNCENABLE, (unsigned long)0);
	}
	if (r != 0)
	{
		codecMutex.Unlock();
...
void AmlCodec::Open(VideoFormatEnum format, int width, int height, double frameRate,bool hasAudio)
{
	if (width < 1)
	...
	InternalOpen(format, width, height, frameRate, hasAudio);
...
void AmlCodec::Reset()
{
	...
	bool hasAudio = this->hasAudioSync;
