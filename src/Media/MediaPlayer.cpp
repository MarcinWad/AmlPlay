...
MediaPlayer::MediaPlayer(std::string url, std::string avOptions, CompositorSPTR compositor, int videoStream, int audioStream, int subtitleStream, std::string alsaDevice)
	:url(url), avOptions(avOptions), compositor(compositor)
{
	...
	//Check if we need audio/video sync in AMCodec
	needAudioSync = false;
	if (source->Outputs()->Find(MediaCategoryEnum::Audio, audioStream))
	{
		needAudioSync = true;
	}
	...
	if (sourceAudioPin)
	{
		...
		audioSink->alsaDevice = alsaDevice;
		audioSink->Execute();
